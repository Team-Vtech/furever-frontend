...
export function useFilters<Filters extends Record<string, unknown>>(queryObj?: Filters) {
    ...
    function add(field: string, value: string, syncUrl: boolean = true) {
        ...
        if (existingFilterId) {
            const updatedFilters = filters.map((filter) => (filter.id === existingFilterId ? createFilter(field, String(value)) : filter));
            if (syncUrl) {
                updateFiltersInUrl(updatedFilters, router, pathname);
            }
        } else {
            const newFilters = addFilter(filters, createFilter(field, String(value)));
            if (syncUrl) {
    ...
    function has(field: string, value: string) {
        return hasFilter(filters, field, value);
    }

    function hasAnyFilter(field: string) {
    ...
    function addFilters(
        ...
        const updatedFilters = filters.map((filter) => {
            const matchingFilter = filtersToAdd.find((newFilter) => newFilter.field === filter.field);
            return matchingFilter ? createFilter(filter.field as string, String(matchingFilter.value)) : filter;
        });

        const newFiltersToAdd = filtersToAdd.filter((newFilter) => !filters.some((filter) => filter.field === newFilter.field));

        const finalFilters = [...updatedFilters, ...newFiltersToAdd.map((filter) => createFilter(filter.field, String(filter.value)))];
