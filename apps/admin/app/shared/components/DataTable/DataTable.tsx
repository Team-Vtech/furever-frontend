...
export function DataTable<TData, TValue>({ data, columns, pagination, filters }: DataTableProps<TData, TValue>) {
    const [columnVisibility, setColumnVisibility] = useState({});
    const searchParams = useSearchParams();
    const router = useRouter();
    const { addFilter, getFilterValue } = useFilters<(typeof filters)["initialData"]>();
    ...
    return (
        <div>
            <div className="flex justify-between py-4">
                <DataTableSearch onChange={(value) => addFilter("search", value)} value={getFilterValue("search") as string} />
                {filters?.config?.length > 0 && <DataTableToolbar config={filters?.config || []} initialData={filters?.initialData || {}} />}
            </div>
