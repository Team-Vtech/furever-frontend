...
export function BookingChartsCard({ statistics }: BookingChartsCardProps) {
    ...
    const serviceData =
        _map(bookings_per_service, (s, key) => ({
            name: s,
            count: key,
        })) ?? [];

    const currentData = activeTab === "providers" ? providerData : serviceData;
    const chartTitle = activeTab === "providers" ? "Bookings per Provider" : "Bookings per Service";
        ...
        <Card className="col-span-2">
            ...
            <CardContent className="space-y-4">
                ...
                <div className="space-y-3">
                    <h4 className="text-muted-foreground text-sm font-medium">{chartTitle}</h4>
