...
export function BookingForm({
  ...
  const serviceById: Service | undefined = services?.find(
    (svc) => svc.id === watchedServiceId
  );
  ...
  return (
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-8">
      ...
      {/* Booking Details */}
      {serviceById
        ? renderBookingSummary(serviceById, watch("addon_ids"))
        : null}
      {/* Form Actions */}
...
function renderBookingSummary(
  ...
  const selectedAddons = addons.filter((addon) =>
    selectedAddonIds?.includes(addon.addon.id)
  );
  const totalPrice = (
