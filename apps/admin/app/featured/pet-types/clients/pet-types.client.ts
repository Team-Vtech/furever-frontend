...
export const PetTypesClient = {
    ...
    async getPetType(id: string) {
        const response = await client().get<JsonResponse<PetType>>(`${ENDPOINTS.getPetTypes.url}/${id}`);
        return response.data;
    },

    async createPetType(data: PetTypeFormValues) {
        const response = await client().post<JsonResponse<PetType>>(ENDPOINTS.getPetTypes.url, data);
        return response.data;
    },

    async updatePetType(id: string, data: PetTypeFormValues) {
        const response = await client().put<JsonResponse<PetType>>(`${ENDPOINTS.getPetTypes.url}/${id}`, data);
        return response.data;
    },

    async deletePetType(id: string) {
        const response = await client().delete<JsonResponse<void>>(`${ENDPOINTS.getPetTypes.url}/${id}`);
        return response.data;
    },
};
