...
import { Edit2, Mail, MapPin, Phone, Trash2, User } from "lucide-react";
...
export function ProviderCard({ provider, onEdit, onDelete }: ProviderCardProps) {
    const getStatusBadge = () => {
        ...
        return (
            <Badge className={`${config.bg} ${config.text} ${config.border} hover:${config.bg} px-3 py-1 text-xs font-medium`}>{config.label}</Badge>
        );
    };
            ...
            <CardContent className="p-6">
                <div className="flex items-start justify-between">
                    ...
                    <div className="ml-4 flex items-center gap-2">
                        {onEdit && (
                            <Button variant="outline" size="sm" onClick={() => onEdit(provider)} className="flex items-center gap-2">
