...
export async function GET(
  ...
  try {
    const response = await (await server()).get(`/api/admin/bookings/${id}`);
...
export async function PUT(
  ...
  try {
    ...
    const response = await (
      await server()
    ).put(`/api/admin/bookings/${id}`, validatedData);
    return NextResponse.json(response.data);
  } catch (error) {
 ...
 * Proxy to backend API: PATCH {{base_url}}/api/admin/bookings/{booking}
 */
export async function PATCH(
  ...
  try {
    ...
    const response = await (
      await server()
    ).patch(`/api/admin/bookings/${id}`, validatedData);
    return NextResponse.json(response.data);
  } catch (error) {
    if (error instanceof z.ZodError) {
      return ValidationError(error);
...
export async function DELETE(
  ...
  const { id } = await params;
  try {
    const response = await (await server()).delete(`/api/admin/bookings/${id}`);
    return NextResponse.json(response.data);
  } catch (error) {
