...
export async function GET(
  ...
  try {
    const response = await (await server()).get(`/api/admin/users/${id}`);
...
export async function PUT(
  ...
  try {
    ...
    const response = await (
      await server()
    ).put(`/api/admin/users/${id}`, validatedData);
    return NextResponse.json(response.data);
  } catch (error) {
...
export async function DELETE(
  ...
  const { id } = await params;
  try {
    const response = await (await server()).delete(`/api/admin/users/${id}`);
    return NextResponse.json(response.data);
  } catch (error) {
