...
export const toastUtils = {
  success: {
    ...
    create: (entityName: string, message?: string) =>
      toast.success(message || `${entityName} created successfully!`),
    /** Show success toast for update operations */
    update: (entityName: string, message?: string) =>
      toast.success(message || `${entityName} updated successfully!`),
    /** Show success toast for delete operations */
    delete: (entityName: string, message?: string) =>
      toast.success(message || `${entityName} deleted successfully!`),
  ...
  error: {
    ...
    create: (entityName: string, error?: unknown) => {
      const message =
        ...
        `Failed to create ${entityName.toLowerCase()}. Please try again.`;
      toast.error(message);
    },
    ...
    update: (entityName: string, error?: unknown) => {
      const message =
        getErrorMessage(error) ||
        `Failed to update ${entityName.toLowerCase()}. Please try again.`;
      toast.error(message);
    },
