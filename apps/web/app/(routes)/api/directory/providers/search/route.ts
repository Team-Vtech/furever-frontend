...
export async function GET(request: NextRequest) {
    try {
        ...
        if (searchQuery.location) {
            ...
            filteredProviders = filteredProviders.filter(
                (provider) =>
                    provider.location.city.toLowerCase().includes(locationLower) || provider.location.state.toLowerCase().includes(locationLower),
            );
        }
        ...
        if (searchQuery.service_type) {
            ...
            filteredProviders = filteredProviders.filter((provider) =>
                provider.services.some((service) => service.name.toLowerCase().includes(serviceTypeLower)),
            );
        }
